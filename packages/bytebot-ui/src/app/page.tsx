...
import { enrichModels, getStatusColor } from "@/utils/modelMetadata";
...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/api/tasks/models")
      .then((res) => res.json())
      .then((data) => {
        // Enrichir les modèles avec descriptions et badges
        const enrichedModels = enrichModels(data);
        setModels(enrichedModels);
        if (enrichedModels.length > 0) setSelectedModel(enrichedModels[0]);
      })
            ...
            <div className="flex w-full max-w-xl flex-col items-center">
              ...
              <div className="bg-bytebot-bronze-light-2 border-bytebot-bronze-light-7 mb-10 w-full rounded-2xl border p-2">
                ...
                <div className="mt-2">
                      ...
                      {models.map((m) => (
                        <SelectItem key={m.name} value={m.name}>
                          <div className="flex items-center gap-2">
                            <span>{m.title}</span>
                            <span className={`text-xs px-2 py-1 rounded font-semibold ${getStatusColor(m.status)}`}>
                              {m.badge}
                            </span>
                          </div>
                        </SelectItem>
                  ...
                  {selectedModel && (
                    <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-950 rounded-lg border border-blue-200 dark:border-blue-800 text-sm">
                      ...
                      <div className={`mt-2 inline-block text-xs px-2 py-1 rounded font-semibold ${getStatusColor(selectedModel.status)}`}>
                        {selectedModel.badge}
                      </div>
                    </div>
                  )}
